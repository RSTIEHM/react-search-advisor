...
const Advisors = () => {
  ...
  useEffect(() => {
    const handleSearch = () => {
      ...
      setFilteredAdvisor(advisorsList);
  ...
  return (
    <div>
      {filteredAdvisor.length > 0 ? (
        <div>
          <input value={query} onChange={(e) => setSearch(e)} type="text" />
          <p>{query}</p>
          <p>{filteredAdvisor.length}</p>

          <ul>
            {filteredAdvisor.map((singleadvisor) => {
              return <li key={singleadvisor.id}>{singleadvisor.name}</li>;
            })}
          </ul>
        </div>
      ) : (
